...
import {
  ...
  ChevronLeft, ChevronRight, Filter, Loader2, ArrowUpDown
} from 'lucide-react'
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="font-serif text-xl font-medium text-white">Productos</h2>
          <p className="text-sm text-white/30 mt-0.5">{total} productos en total</p>
        </div>
        <Link
          href="/admin/products/new"
          className="inline-flex items-center gap-2 h-10 px-4 rounded-xl bg-white text-black text-sm font-semibold hover:bg-white/90 active:scale-[0.98] transition-all"
        >
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <select
          ...
          className="bg-[#0d0d0d] border border-[#1a1a1a] rounded-xl h-10 px-3 text-sm text-white/60 focus:outline-none focus:border-white/15 transition-all cursor-pointer"
        >
      ...
      <div className="bg-[#0d0d0d] border border-[#1a1a1a] rounded-2xl overflow-hidden">
        {loading ? (
          ...
          <div className="py-20 text-center">
            ...
            {!search && (
              <Link href="/admin/products/new" className="inline-flex items-center gap-2 h-9 px-4 rounded-xl bg-white text-black text-xs font-semibold hover:bg-white/90 transition-all">
              ...
              <table className="w-full min-w-[700px]">
                <thead>
                  <tr className="border-b border-[#1a1a1a]/60">
                    <th className="text-left px-5 py-3.5 text-[10px] font-semibold uppercase tracking-wider text-white/25">Producto</th>
                    <th className="text-left px-5 py-3.5 text-[10px] font-semibold uppercase tracking-wider text-white/25 hidden md:table-cell">Categoría</th>
                    <th className="text-left px-5 py-3.5 text-[10px] font-semibold uppercase tracking-wider text-white/25">Precio</th>
                    <th className="text-left px-5 py-3.5 text-[10px] font-semibold uppercase tracking-wider text-white/25">Estado</th>
                    <th className="text-center px-5 py-3.5 text-[10px] font-semibold uppercase tracking-wider text-white/25">Stock</th>
                    <th className="text-right px-5 py-3.5 text-[10px] font-semibold uppercase tracking-wider text-white/25">Acciones</th>
                  </tr>
                </thead>
                      ...
                      <tr key={product.id} className="hover:bg-white/[0.02] transition-colors group">
                        <td className="px-5 py-3.5">
                          <div className="flex items-center gap-3.5">
                            <div className="w-11 h-11 rounded-xl bg-[#111] border border-[#1a1a1a] overflow-hidden flex-shrink-0">
                              {thumb ? (
                            ...
                            <div className="min-w-0">
                              <div className="flex items-center gap-2">
                                <span className="text-sm font-medium text-white/90 truncate max-w-[200px]">{product.name}</span>
                                {product.featured && <Star className="w-3.5 h-3.5 text-[#bf9b4e] fill-[#bf9b4e] flex-shrink-0" />}
                              </div>
                              {hasDiscount && (
                                <span className="inline-flex items-center px-1.5 py-0.5 rounded bg-[#e05555]/10 text-[10px] font-bold text-[#e05555] mt-1">
                        ...
                        <td className="px-5 py-3.5">
                          <div>
                            ...
                            {hasDiscount && (
                              <span className="block text-[10px] text-white/25 line-through mt-0.5">{formatARS(product.discountPrice)}</span>
                            )}
                          </div>
                        </td>
                        <td className="px-5 py-3.5">
                        ...
                        <td className="px-5 py-3.5 text-center">
                          {product.status === 'OUT_OF_STOCK' ? (
                            <span className="text-xs text-[#e05555]">0</span>
                          ) : product.status === 'PREORDER' ? (
                            <span className="text-xs text-[#d4a030]">Por pedido</span>
                          ) : (
                        ...
                        <td className="px-5 py-3.5">
                          <div className="flex items-center justify-end gap-1">
                            <Link
                              ...
                              target="_blank"
                              className="p-2 rounded-lg text-white/20 hover:text-white/70 hover:bg-white/[0.04] transition-all"
                              title="Ver en tienda"
                            >
              ...
              <div className="flex items-center justify-between px-5 py-4 border-t border-[#1a1a1a]">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    disabled={page <= 1}
                    className="flex items-center gap-1.5 h-9 px-3.5 rounded-xl bg-[#111] border border-[#1a1a1a] text-xs text-white/50 hover:text-white hover:border-white/10 transition-all disabled:opacity-30 disabled:cursor-not-allowed"
                  >
